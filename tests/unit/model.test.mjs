...
const bundle = await build({ entryPoints: ['web/src/model.ts'], bundle: true, format: 'esm', write: false });
...
test('browser dataset preserves all source percentages and group assignments', () => {
  ...
  const csv = readFileSync('reports/python/plotted_data.csv', 'utf8').trim().split(/\r?\n/).slice(1);
...
test('invalid counts, inconsistent metadata and duplicates fail validation', () => {
  const mutations = [
    ...
    d => { d.rows[0].total = 10; d.rows[0].percent = 100 * d.rows[0].positive / 10; },
    d => { d.rows[30].group = (d.rows[30].group + 1) % 3; }, d => { delete d.tissueNames.HP; },
